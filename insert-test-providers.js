...
async function insertProviders() {
	return new Promise((resolve, reject) => {
		db.serialize(() => {
			// 先删除已存在的测试数据
			db.run(
				`DELETE FROM platform_ai_provider WHERE provider_key IN ('openai', 'anthropic')`,
				(err) => {
					if (err) {
...
insertProviders()
	.then(() => {
		console.log('\n📊 验证数据...');
		db.all(
			'SELECT provider_key, provider_name, is_active, enabled FROM platform_ai_provider',
			(err, rows) => {
				if (err) {
					...
					rows.forEach((row) => {
						console.log(
							`   - ${row.provider_name} (${row.provider_key}): active=${row.is_active}, enabled=${row.enabled}`,
						);
					});
				}
				db.close();
