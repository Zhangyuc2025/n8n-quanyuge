...
# 注意：这些命令可以重复运行，会自动停止旧进程
```

**智能启动特性：**
- ✅ **自动停止旧进程** - 重复运行命令会自动停止旧的开发服务器
- ✅ **低内存占用** - 仅启动必要进程（约 4-6 个 TypeScript 编译进程）
- ✅ **避免 OOM** - 16GB 内存足够，不会像之前启动 115+ 进程导致内存溢出
- ✅ **无端口冲突** - 自动清理占用端口的进程
