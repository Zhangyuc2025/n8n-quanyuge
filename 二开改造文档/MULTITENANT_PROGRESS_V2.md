...
> **更新时间:** 2025-10-29
> **当前状态:** ✅ Phase 3 完成 - 团队API层全部实现（19个端点）
> **当前方案:** 基于 Project 架构扩展（最小改动策略）+ 完整计费系统
> **预计总工期:** 10-11 周（已调整，含完整计费）
> **当前版本:** n8n@1.117.0 (满血版)
> **下一步:** Phase 3.5 - 注册登录改造（多租户SaaS必备，侧边栏首页+弹窗登录）
> **方案版本:** v2.2（2025-10-29新增注册登录改造）
...
### 📋 Phase 3.5: 注册登录改造（Week 6.5）⭐ 新增 - 待开始
...
- [ ] **扩展 UserService.registerTenant() 方法**
  - [ ] 创建用户时自动设置 tier='free', tenantStatus='active'
  - [ ] 自动创建默认个人工作区（name="我的工作区", type='personal', isDefault=true）
  - [ ] 自动创建 ProjectRelation（用户是 project:admin）
  - [ ] 记录注册事件到 EventService
  - [ ] 简化版：firstName = username, lastName = ''

- [ ] **扩展 AuthController 登录响应**
  - [ ] POST /auth/register - 简化注册API（邮箱+用户名+密码）
  - [ ] POST /auth/login - 返回用户信息+工作区列表+默认工作区
  - [ ] 实现 getWorkspaces() 私有方法（获取个人+团队工作区）
  - [ ] 邮箱验证：基本格式即可
  - [ ] 用户名验证：至少2个字符
  - [ ] 密码验证：至少6位（无复杂度要求）
